...
```bash
python src/replay_rerun.py                 # 存 out/replay.rrd,Rerun 查看器打开
python src/replay_rerun.py --serve         # 起 web,浏览器开打印的完整 URL
python src/replay_rerun.py --traj a=out/robot_traj_raw.pkl --traj b=out/robot_traj.pkl   # A/B 对比
```
...
`assets/assembled/nero_inspire_right.urdf`(装配)、`out/canonical_ds`、`out/lerobot_ds_*`、`out/robot_traj_*.pkl`、`out/*.rrd`。
