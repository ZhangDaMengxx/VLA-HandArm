...
自检:
    /usr/bin/python3 src/skills/hand_pose.py            # 手势表 + 可行域
    /usr/bin/python3 src/skills/hand_pose.py --verify    # 与 inspire_hand 核对
"""
...
# ⚠ 不是 src/schema.py 的 HAND_ACTUATED 顺序 —— 那个是四指在前、拇指在后。
...
def verify_against_driver() -> list[str]:
    """按**文件路径**加载 src/inspire_hand.py 逐项核对。返回不一致列表。

    不用 `import inspire_hand` —— 那要把 src/ 塞进 sys.path,而 src/schema.py 会
