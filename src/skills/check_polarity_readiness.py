...
用法:
  python3 src/skills/check_polarity_readiness.py          # 报告
  python3 src/skills/check_polarity_readiness.py --strict # C1/C2 不过就退出码 1
"""
...
# 只把 skills/ 放进 path,**不要**放 src/ —— src/schema.py 和 skills/schema.py 撞名,
# 放了 src/ 就会 import 到错的那个(intent.py 也是只放 skills/,保持一致)。
