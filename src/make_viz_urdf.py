"""把 assets/assembled 里的装配 URDF 转成相对 mesh 路径副本,供 VSCode 查看。

为什么要副本:
- pinocchio(replay_rerun 用)吃绝对路径最稳,原文件保持绝对路径不动。
- VSCode 插件把 mesh filename 当相对 URL 拼在 URDF 目录后面,绝对路径会被拼成
  `assets/assembled//home/...` 而 404。副本改成相对顶层 assets/ 的路径即可。

用法: python3 src/make_viz_urdf.py   然后在 VSCode 里打开 *_viz.urdf
"""
