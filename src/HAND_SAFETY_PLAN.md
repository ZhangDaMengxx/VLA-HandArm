...
`replay_rgb_demo`(`voice_enabled: true`)指向 `src/out/robot_traj_nero_inspire_rgb.npz`,
逐帧过 `check_feasible`:
...
**为什么发得出去**:`check_feasible` 只在 `src/skills/schema.py:299` 被调用 ——
...
**2026-08-11 实施:`src/collision_checker.py`(~330 行)。检查器可用,但 URDF 不可信。**
...
**2026-08-11 更新:无需第 2 步真机。** 用户提醒:手只报 raw,报不出 rad,
上外部量具才能定 rad。改为**从实测边界反推**:`calibrate_thumb_chain.py`
拟合旧表三点 → `thumb_2` span = **0.525 rad**(比 URDF 0.48 大 9%,
比 xlsx 0.698 小 25%),`thumb_3` mimic 系数 k3=**1.075**(URDF 1.1425 偏紧 6%)。

标定后重跑自检:

| T | 实测堵转 | 几何接触 | 差(counts) | 判据一 |
|---|---|---|---|---|
| 300 | 225 | 610 | 385 | ✓ 保守 |
| 450 | 52 | 70 | 18 | ✓ 保守(紧) |
| 600 | 0 | 全程不碰 | — | ✓ |

判据 1/3/4 通过,判据 2 要两力档真机。**第 3 步验收通过,可进第 4 步。**
