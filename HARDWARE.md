...
锚定后的有限范围相对移动，不是深度相机提供的绝对米制位置。Mock 默认末端相对位置限幅为
`±50/50/30mm`，真臂路径收紧为位置各轴 `±20mm`。机械臂末端姿态在锚点附近按
`±45/±25/±35°` 限幅后跟随腕部相对旋转；掌心朝向会参与姿态估计并驱动有限范围的末端旋转。
...
Z `-175/+155°` 的非对称范围；
真机仍使用保守的 `±45/±25/±35°`。翻掌时的单目深度使用 MediaPipe world landmarks
