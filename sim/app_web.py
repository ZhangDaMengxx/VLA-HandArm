...
# ⚠ 坑:下面这几行会让 backend/runner 在模块级把 sim/skills 和 sim 都插进 sys.path,
#   于是本进程里裸 `import schema` 解析到的是 **skills/schema.py**(backend 先把它
#   缓存进 sys.modules),不是数据集那份 sim/schema.py。app_web 不用数据集 schema,
...
def run_pipeline(input_path: str | None, skip_regen: bool, dataset: str, emit,
    ...
    if input_path and Path(input_path).exists() and not is_rgbd and source != "handfile":
        out_vid = REPO / f"sim/out/replay_video_original_{robot}.mp4"
        try:
    ...
    missing = [str(p.name) for p in (TRAJ_NPZ, REPO / "sim/out/canonical_ds/meta/info.json")
               if not p.exists()]
    if missing:
    ...
    def start(self, mock: bool = False) -> None:
        ...
        **不再起 hand_rerun** —— 3D 改成浏览器端 three.js 直接渲染(web/hand3d.js),
        ...
        self.console = subprocess.Popen(
            ["python3", "sim/hand_console.py", flag,
...
class ArmDebugSession:
    ...
    def start(self, mock: bool = True, speed: int = 20) -> None:
        ...
        self.console = subprocess.Popen(
            ...
            ["python3", "sim/arm_console.py", flag, "--hz", "20",
             "--speed", str(int(speed))],
            cwd=str(REPO), stdin=subprocess.PIPE, stdout=subprocess.PIPE,
            stderr=subprocess.DEVNULL, text=True, bufsize=1)
        self._threads = [threading.Thread(target=self._pump_console, daemon=True)]
...
# 没生成过就先跑 `python3 sim/build_arm_viz.py`。
...
# 没生成过就先跑 `python3 sim/build_combo_viz.py`。
...
async def replay_keypoints(robot: str = "") -> JSONResponse:
    ...
    ds_root = REPO / "sim/out/canonical_ds"
    ...
    src_w = src_h = 0
    orig = REPO / f"sim/out/replay_video_original_{robot}.mp4" if robot else None
...
async def replay_video_canonical():
    ...
    p = REPO / "sim/out/canonical_ds/videos/observation.images.ego/chunk-000/file-000.mp4"
    if not p.exists():
...
async def replay_video_original(robot: str = ""):
    ...
    p = REPO / f"sim/out/replay_video_original_{robot or 'default'}.mp4"
    if not p.exists():
...
async def hand_start(mock: bool = False) -> JSONResponse:
...
async def hand_action_start(slot: int) -> JSONResponse:
...
async def arm_start(mock: bool = True, speed: int = 20) -> JSONResponse:
