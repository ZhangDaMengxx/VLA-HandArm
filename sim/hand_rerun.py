#!/usr/bin/env python3
"""sim/hand_rerun.py — 手部可视化:stdin 手关节流 → pinocchio FK(含 mimic)→ Rerun serve。
...
用法(一般由 app_web.py 调):
  python3 sim/hand_console.py | python3 sim/hand_rerun.py --serve
"""
...
# ⚠ 和 sim/web/hand3d.js 的 MIMIC 是同一份数据,改一处要改两处。
MIMIC = {
