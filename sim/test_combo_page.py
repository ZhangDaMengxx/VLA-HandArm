#!/usr/bin/env python3
"""sim/test_combo_page.py — 合体页(实时 Live)的结构性测试。
...
"错了立刻报错"的那类。后者不用测,前者测不到就会在页面上以"看起来没加载"的形式出现。
    ...
    python3 sim/test_combo_page.py
"""
...
WEB = SIM / "web"
COMBO = SIM / "assets/combo_viz"
COMBO_URDF = COMBO / "nero_inspire_right_viz.urdf"
SRC_URDF = SIM / "assets/nero_inspire_right.urdf"
...
def test_combo3d_imports_mimic_instead_of_copying():
    ...
    已经有 sim/hand_rerun.py 和 web/hand3d.js 两份要同步了。抄第三份的话症状是
