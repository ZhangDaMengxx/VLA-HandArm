#!/usr/bin/env python3
"""sim/build_combo_viz.py — 把**装配** URDF(臂+法兰+手)转成浏览器能吃的一份。
...
挂载根内的相对路径、collision 整段删掉。产物 `sim/assets/combo_viz/`。
...
输出:
  sim/assets/combo_viz/nero_inspire_right_viz.urdf  +  meshes/*.glb
"""
...
def main() -> int:
    if not SRC_URDF.is_file():
        print(f"找不到装配 URDF {SRC_URDF}\n先跑:python3 sim/build_nero_inspire.py",
              file=sys.stderr)
        return 1
