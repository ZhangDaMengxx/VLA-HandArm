#!/usr/bin/env python3
"""sim/analyze_traj_feasible.py — 轨迹可行域诊断(纯离线,不碰硬件)。
...
用法:
    /usr/bin/python3 sim/analyze_traj_feasible.py              # 全部 robot_traj_*.npz
    /usr/bin/python3 sim/analyze_traj_feasible.py --npz <path> # 只看一个
"""
...
sys.path.insert(0, str(REPO / "sim" / "skills"))
...
def main() -> int:
    ...
    if args.npz:
        ...
        paths = sorted((REPO / "sim" / "out").glob("robot_traj_*.npz"))
        if not paths:
            print("✗ sim/out/ 下没有 robot_traj_*.npz")
            return 1
