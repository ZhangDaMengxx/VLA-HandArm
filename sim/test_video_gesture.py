...
def test_console_hz_matches_spawn_arg():
    ...
    src = (Path(__file__).resolve().parent / "app_web.py").read_text(encoding="utf-8")
    m = re.search(r'"sim/hand_console\.py".{0,200}?"--player-hz",\s*([^\]]+?)\]',
                  src, re.S)
    ...
    src_console = (Path(__file__).resolve().parent
...
def test_no_savgol_smoothing():
    ...
    src = (Path(__file__).resolve().parent / "video_gesture.py").read_text("utf-8")
