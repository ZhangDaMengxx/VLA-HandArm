...
- 6dof 手部重定向代理模型新增基于 vendor STL 的 `thumb_tip`、`index_tip`、
  `middle_tip`、`ring_tip`、`pinky_tip` 固定指尖 frame；可视化用的完整 mimic URDF
  保持不变
- 合体页真机腕部跟随改用与 Mock 相同的位置和姿态安全范围，保留 IK、关节限位、真实臂
  显式授权和冻结门控
- 实时手部 retarget 的 `low_pass_alpha` 从 `0.2` 调整为 `1.0`，关闭逐帧固定低通；真机
  继续使用每个 WebSocket 独立的 One Euro，避免两层滤波叠加响应迟滞

### Fixed (修复)

- 修正目标 frame 停在远端指骨造成的指尖几何偏差；离线视频仍能观察到稳定的张开残差，
  后续需用 Inspire 专用的多指节屈曲映射继续解决，不能把该残差归因于 MediaPipe 抖动
- 恢复腕部完整翻转所需的姿态输入范围，避免真机分支的旧保守限幅提前裁剪手背翻转
- 修复合体摄像头关闭或网络断开恰逢服务端回包时，断连被误报为跟随失败并再次向已关闭
  WebSocket 发送响应的问题；关闭竞争现在进入正常断连清理

### Safety (安全)

- 真机现在允许更大的末端位姿变化，可能更接近工作空间边界、奇异位形、碰撞和线缆限制；
  仍必须在低速、净空和急停可用条件下现场验证，Mock 通过不代表真机可达

### Verification (验证)

- 6dof URDF XML、retargeter 构建、视频离线抽取以及实时手/腕相关回归通过；WebSocket
  关闭竞争回归在内的定向测试 `68 passed`
