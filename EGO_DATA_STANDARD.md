...
| 项 | 目标 |
|---|---|
| RGB | 至少 1280x800、标称 60 fps；设备时间戳实测 `>=59.4 Hz` |
| 原生 Depth | 至少 848x480、标称 60 fps；设备时间戳实测 `>=59.4 Hz` |
...
`ego_fixed_rgbd_60hz_v1` 只适用于达到该能力的固定设备。RGB 和 raw Depth 的 60 fps 是
硬指标：必须显式选择 60 fps profile，并以各自设备时间戳证明实际 cadence 达到标称值的
99%（`>=59.4 Hz`）；缺时间戳、仅在配置中声明 60、丢帧后平均不足或退回 30 fps 均失败。
旧 30 fps 数据或未来眼镜不得靠补帧套用该 Profile。
