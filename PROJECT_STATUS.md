...
- Web 硬件控制新增 hand/arm 独立 owner、2 秒 heartbeat 和服务端 8 秒租约 watchdog；
  第二标签页点击接入会保持位置断开对应旧通道并成为新 owner，不影响另一硬件通道
...
8. 页面切换、关闭和服务端租约已覆盖正常释放、重复标签页 owner 替换及超时兜底；进程强杀、
   主机断电、断网和 owner 替换的真机现场行为仍需验证，watchdog 不能替代急停。
